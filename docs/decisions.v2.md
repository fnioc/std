...
surface: `types` → `./src/*.ts`, `bun` → the package's per-file lowered stage emit. The emit
directory itself is a build implementation detail, not part of this rule — the export alias and the
disk path are independent. The root `.` export carries `types` +
...
`exports` down to `.` alone, and `files` excludes the stage-emit directory.
