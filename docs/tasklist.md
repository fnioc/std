...
- `validateBuildability()` misses captives in shadowed registrations: LANDED `4173ca59` — build validation plans the newest registration through `Plan.from`, then every registration it shadows through `Plan.fromShadowed`; nothing memoizes a shadowed plan. Consequence for the owner's word: a shadowed registration whose own dependencies are unregistered now fails the build too (before: planned nothing, the collection ask silently omitted it). No existing use of `validateBuildability`/`validateOnBuild` in the di or hosting test packages is affected. Tests written, not yet run.
