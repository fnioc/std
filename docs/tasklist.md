...
Ruled 2026-09-03 ("src it is"): every `*.extras` publishes `src/` beside dist, with a `source` condition on the `.` export pointing at `./src/index.ts` for the inline stage; the runtime target stays on dist. The build step stays — nothing in an extras package runs untransformed (`typefor`/`schemaof` throw, `registerInlineBodies` is empty, every sugar body routes through them), so dist's whole value is the rolled `.d.ts` plus the runtime stub whose one behavior is the "transform not applied" throw. Same directory, two readers. Landed `798f496d`. Consumer proof from pnpm-packed tarballs: `manifest.add<IGreeter>(Greeter, 'singleton')` inlines to the explicit four-argument form with both `typefor` calls folded; hiding the shipped `src` leaves the sugar unlowered, which is the control.

Alongside it (owner, same /go): audit every library's `exports`/`publishConfig.exports` for redundancy — conditions repeated where the fallback would resolve identically — and make them ideal. Landed `bb87d5cf`. The published `.` is a bare `./dist/bundle/index.js` string (an extras adds `source` ahead of it): `import` duplicated `default` on an ESM-only bundle, and `types` restated the `.d.ts` sitting beside that bundle. The top-level `types` field went with them, `main` staying for node10-style resolvers. `./private/*` collapses to its bare `./src/*.ts` target. Proven on the packed tarballs: `tsc --noEmit` is clean under `bundler`, `node16` and `node10`, and arethetypeswrong resolves types for `di` and `di.extras` in every mode.
...
Status: landed in 3308a79a (code + tests) and e892e69a (docs), corrected in 63202a94 + 768d5a44. Reaching the async-node condition with no enclosing await is a plain miss, not an error: an optional dependency falls back to `undefined`, a required one is unsatisfiable.
...
- `validateBuildability()` misses captives in shadowed registrations: LANDED `4173ca59` — build validation plans the newest registration through `Plan.from`, then every registration it shadows through `Plan.fromShadowed`; nothing memoizes a shadowed plan. RULED (owner 2026-09-04): a shadowed registration's dependencies follow the same recursive planning as any other registration's, so one whose own dependencies are unregistered fails the build. `Plan.fromShadowed` runs the same `PlannerVisitor` walk as `Plan.from`; the code conforms. Tests green in the 2026-09-03 full gate.
- A promise settling after its scope ended fulfils with a disposed instance: LANDED `7a224907` — the standard model's `canonicalize` swaps a thenable construction for a derived promise; on settlement the value is filed with the owning scope, and an owner that ended meanwhile disposes the value and the derived promise rejects with `ObjectDisposedError` (no new class). The two existing disposal-on-settlement tests awaited fulfilment, which the ruling forbids; they now assert rejection and keep their disposal assertions. Tests written, not yet run.
- Collection ask cached as a whole: RULED — "arrays registered as a whole array get disposed as a whole, individual = individual." A registered array value is one service with one lifetime; a collection ask is fresh per ask and each element honours its own registration; the synthesised collection node is never cached. Differs from the reference's array-identity caching by the owner's word. Pinned by tests, LANDED `f5dd4aa4`; the engine matched on every point, nothing changed. Tests written, not yet run.
