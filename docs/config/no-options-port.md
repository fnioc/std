...
- **The three accessor types are three different DI lifetimes over that
  ...
  - `IOptionsSnapshot<T>` → `OptionsManager<T>`, **scoped** — re-bound
    once per DI scope (an ASP.NET Core request, typically), and the one
...
| `IOptionsSnapshot<T>` (scoped, named)                              | `OptionsManager<T>` registered scoped; `Get(name)`                                                                                                                                           | "Scoped" is an ASP.NET Core DI-container lifetime; no equivalent scope exists in a DI-free library                                                                                                                                                                                                                          | N/A                                                                                                                                                                                                                                                                                                                       |
