...
| resolving from a scope that was already open when the root was disposed | `ObjectDisposedException` — `ServiceProviderEngineScope.GetService` checks its own `_disposed`, then forwards to `ServiceProvider.GetService(ServiceIdentifier, ServiceProviderEngineScope)`, which checks the root's `_disposed` (`Microsoft.Extensions.DependencyInjection/src/ServiceProvider.cs`). Root disposal walks no scope list: the open scope's own tracked instances are disposed only when that scope itself is disposed |
