...
| -- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1  | **Keyed-service surface** — **correction: `[recorded]`, not silent.** `docs/decisions.v2.md` §85 ("Keyed services as token-key composition") is an owner-approved design for `IKeyedServiceProvider`/`ServiceKeyAttribute`/`ServiceKeyLookupMode`/`FromKeyedServicesAttribute`, the `AddKeyed*` overload ladder, `TryAddKeyed*`/`RemoveAllKeyed`, `GetKeyedService*`, and `IServiceProviderIsKeyedService`, and has since landed.                                                                      | di / di.core   | None — a port path exists and is tracked at §85, superseding the narrower `ActivatorUtilities` note (decisions.md §56) this row previously leaned on; that note is moot regardless, since §128 removed the activation surface it described.                                                                      |
...
| 7  | **`ActivatorUtilities` generic-`T` convenience overloads** (`CreateInstance<T>()`, `GetServiceOrCreateInstance<T>()`) — **correction: MOOT, the whole activation surface is gone.** `docs/decisions.v2.md` §128 removed `ActivatorUtilities`, `ObjectFactory`, and `ActivationError` outright as porting noise, so there is no non-generic core left for these to be ergonomic sugar over.                                                                                                             | di.core        | None — the gap closed by deletion rather than by porting. Left at #7 only to keep the ranking's numbering stable for the cross-references elsewhere in this file.                                                                                                                                                |
...
| Reference member                                                                                                                                                                                                                                                                                                  | Status                                                                    | Class                              | Evidence                                                                                                                                             |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IServiceProvider`/`IResolver`/`IScopeFactory`/`IRequiredResolver`/`IServiceQuery`, `ServiceCollectionDescriptorExtensions` (`TryAdd*`, `Replace`, `RemoveAll`), `EmptyServiceProvider`, `ISupportRequiredService`→`IRequiredResolver`, `IServiceProviderIsService`→`IServiceQuery`, `IServiceProviderFactory<T>` | ported                                                                    | —                                  | full parity; §10/§27/§23/§56/§24                                                                                                                     |
...
| **Entire activation surface** (`ActivatorUtilities` core verbs + `CreateInstance<T>()`/`GetServiceOrCreateInstance<T>()` generic overloads, `ObjectFactory`, `[ActivatorUtilitiesConstructor]`)                                                                                                                   | ported, then REMOVED                                                      | `[recorded]`                       | §128 — owner-directed deletion as porting noise; the core verbs did land under §56, which §128 supersedes on this point                              |
| **Entire keyed-service surface** (`IKeyedServiceProvider`, `ServiceKeyAttribute`, `ServiceKeyLookupMode`, `FromKeyedServicesAttribute`, `AddKeyed*`, `TryAddKeyed*`/`RemoveAllKeyed`, `GetKeyedService*`, `IServiceProviderIsKeyedService`)                                                                       | missing                                                                   | `[silent]`                         | see [Alarm §1](#alarm--silent-gaps); the narrower `ActivatorUtilities` parameter-matching exclusion (§56) is moot now that §128 removed that surface |
