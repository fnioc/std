...
// (the `removeAll`/`tryAdd*`/`replace*` registration verbs) into this bundle, and the
...
// di.core external is what makes the registration verbs chainable downstream.
...
export default { input: join(PKG_ROOT, 'src', 'index.ts'), output: { file: join(PKG_ROOT, 'dist', 'bundle', 'index.d.ts'), format: 'es' },
  external: [/^@rhombus-std\/di\.core$/, /^@rhombus-std\/primitives$/], plugins: [dts({ tsconfig: join(PKG_ROOT, 'tsconfig.json'), respectExternal: true })] };
