...
const config = new ConfigBuilder().addCommandLine(process.argv.slice(2), { '-p': 'Server:Port' }).build();
...
```ts
new ConfigBuilder().addCommandLine(process.argv.slice(2), { '-p': 'Server:Port', '-h': 'Server:Host' });
```
