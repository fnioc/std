...
export namespace ServiceManifestOptionsAugmentations {
  ...
  export function postConfigure<T, Deps extends readonly unknown[], S extends string = string>(this: Manifest<S>,
    ...
    if (Array.isArray(step)) {
      ...
      const depTypes = (step as readonly (Type | string)[]).map(dep => typeof dep === 'string' ? Type.from(dep) : dep);
...
declare module '@rhombus-std/di.core' {
  interface Manifest<Scopes extends string = any> extends Flatten<typeof ServiceManifestOptionsAugmentations> {}
}
