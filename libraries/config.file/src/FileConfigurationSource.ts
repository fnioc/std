// FileConfigurationSource -- the abstract base for every file-backed
// configuration source (JSON, INI, XML). Ported from the reference
// `FileConfigurationSource`. Holds the file provider + path + optionality +
// reload settings a concrete source's `build` turns into a
// FileConfigurationProvider, and the two builder-default hooks
...
/** The base class for file-based {@link IConfigurationSource} implementations. */
export abstract class FileConfigurationSource implements IConfigurationSource {
   ...
   * caller left unset.
   */
  public ensureDefaults(builder: IConfigurationBuilder): void {
