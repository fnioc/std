...
export default { input: join(PKG_ROOT, 'src', 'index.ts'), output: { file: join(PKG_ROOT, 'dist', 'bundle', 'index.d.ts'), format: 'es' },
  external: [/^@rhombus-std\/config$/, /^@rhombus-std\/config(\/|$)/, /^@rhombus-std\/config\.core$/, /^@rhombus-std\/fileproviders\.core$/, /^@rhombus-std\/fileproviders\.physical$/,
    /^@rhombus-std\/primitives$/, /^@rhombus-std\/primitives\.extras(\/|$)/, /^@rhombus-toolkit\/types$/], plugins: [dts({ tsconfig: join(PKG_ROOT, 'tsconfig.json'), respectExternal: true })] };
