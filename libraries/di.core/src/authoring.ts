...
//
// `ServiceManifest` is an OPEN receiver: a downstream family contributes its own
// fluent verb (`services.addMyThing(...)`) by registering an augmentation set
// against the shared `ServiceManifest` token — see docs/features/augmentations.md
// for the authoring steps. A manifest is IMMUTABLE, so such a verb must hand back
// the manifest its registrations produced — never `this`, and never a discarded
// intermediate.
 ...
 *
 * `Gated` says whether a dependency signature is still owed:
 *
 * - **`Gated = true`.** A signature is mandatory. While the `'signatures'` slot is
 *   still present — meaning NO signature has been supplied yet, the state of the
 *   bare 2-arg `addClass(token, ctor)` form — the MANIFEST FACE IS WITHHELD
 *   (`'signatures' extends Slots ? unknown : …`), so `build` / `addClass` / `seal`
 *   are absent and the chain cannot be finished. `withSignature` /
 *   `withSignatures` supplies a signature, strikes `'signatures'`, and the
 *   manifest face reappears. `.as()` / `.withKey()` refine WITHOUT opening the
 *   gate (they leave `'signatures'` in place). Supplying the signature
 *   POSITIONALLY (the 3+-arg overloads) strikes `'signatures'` up front, so those
 *   chains start ungated.
 * - **`Gated = false`.** The signature comes from the type argument, so the
 *   manifest is ALWAYS present and `withSignature` / `withSignatures` are
 *   OVERRIDES, not a gate.
 *
 ...
 * additional overloads to the one given: `addClass(t, c, [[…]]).withSignature('a')`.
 * When `Gated = false` it appends onto the derived signature instead.
 */
 ...
 * its effective token as `base#key`. Because the recomposed token is
 ...
 * impl class, because a downstream package contributes extra authoring overloads
 * by DECLARATION-MERGING onto this interface — merged overloads land on an
 * interface, never on a class.
 *
  ...
  extends Iterable<ManifestEntry>
{
  ...
  addClass(token: Token, ctor: Ctor, signatures: DepSignatures,
    scope: Scopes): AddChain<Scopes, 'signature' | 'key', true>;
  addClass(token: Token, ctor: Ctor, signatures: DepSignatures, scope: Scopes,
  ...
  addFactory(token: Token, factory: Factory,
    signatures: DepSignatures): AddChain<Scopes, 'signature' | 'scope' | 'key', true>;
  addFactory(token: Token, factory: Factory, signatures: DepSignatures,
    scope: Scopes): AddChain<Scopes, 'signature' | 'key', true>;
  addFactory(token: Token, factory: Factory, signatures: DepSignatures, scope: Scopes,
    key: string): AddChain<Scopes, 'signature', true>;
  /**
   ...
   * composes a keyed registration token `base#key`. Returns the new manifest with
   * no modifier faces: there is no slot left to fill.
   */
...
// `IServiceManifest<S>` — this interface bound to the `IServiceProvider<S>` that
// `build()` returns — lives in `./IServiceManifest.ts` beside the class that
// implements it. The constructible `ServiceManifest` VALUE lives in
// `@rhombus-std/di`, which also patches a working `build()` onto that class.
