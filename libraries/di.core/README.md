...
| `LifetimeModel<Lifetime>`                       | The scope/lifetime pattern a container runs on. `@rhombus-std/di`'s `noop()` is the built-in model that retains nothing — every registration is constructed fresh every time.                                                                                                |
...
| `ResolveAudit`                                  | Access to the resolve that constructed the holder — what was asked for, which construction the holder fills, and what encloses it. Name `ResolveAudit` in a constructor's signature to receive one.                                                                          |
