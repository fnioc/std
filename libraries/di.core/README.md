...
| `DiError` and its subclasses                                                                            | Classes              | The whole error taxonomy — the `DiError` root, the registration-time `OpenTokenRegistrationError`, and the resolution-time errors `@rhombus-std/di` raises (`UnregisteredTokenError`, `OpenTokenResolutionError`, `CircularDependencyError`, `MissingMetadataError`, `NoSatisfiableSignatureError`, `NoSatisfiableUnionError`, `FactoryTargetError`, `AsyncResolutionRequiredError`, `AsyncDisposalRequiredError`, `RegistrationValidationError`, `ScopeValidationError`, `ProviderDisposedError`). Declared here so a library can classify a container failure without referencing the engine. |
...
**Closed-generic grammar:** `base<arg1,arg2>` — the canonical form has no whitespace around `<` `>` `,`, and whitespace you do write is stripped when the token is parsed. Each arg is itself a token, so nesting recurses (`pkg:IFoo<pkg:IBar<./src/Baz>>`). A **hole** is an arg that is exactly `$N` — decimal, `N ≥ 1`, no leading zero, so `$0`, `$01` and `$007` are ordinary text rather than holes. A token containing a hole at any depth is an _open template_. Literal-type args keep their interior spaces/quotes (`"a" | "b"`) — the parser is quote-aware, so commas and angle brackets inside double quotes never count as separators.
...
unkeyedToken('pkg:IRepository<$1>#redis'); // "pkg:IRepository<$1>"
```
...
- `isOpenToken(token)` is `true` when `token` contains a hole at any depth. It answers off the PARSED token, so it does not depend on spelling — `pkg:IMap<string, $1>` is the open template it canonicalises to — and it reads the hole grammar straight back off the parser rather than restating it, so there is no second opinion on what a hole is. `pkg:IMap<$0>` and `pkg:IMap<$01>` are closed: not holes, so not templates.
- `unkeyedToken(token)` drops a trailing `#key`, returning `token` unchanged when it has none. Classify a possibly-keyed token as `isOpenToken(unkeyedToken(token))` — a key can neither introduce nor remove a hole, and stripping it first keeps the two spellings of one keyed registration in agreement.
