...
- `isOpenToken(token)` is `true` when `token` contains a hole at any depth. It answers off the PARSED token, so it does not depend on spelling: `pkg:IMap<string, $1>` and `pkg:IMap<$01>` are the open templates they canonicalise to. (`$0` is the one exception — hole labels are 1-based, so `$0` is an ordinary arg, not a hole.)
