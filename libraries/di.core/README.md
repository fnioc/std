...
| `closeToken` / `parseToken` / `isOpenToken` / `unkeyedToken` / `typeArg`                                | Functions            | The open-generic STRING grammar: render, parse, detect a closed/open generic token, drop a token's trailing `#key` before classifying it, and mint a `{ typeArg: n }` slot by hand.                                                                                                                                       |
...
- `unkeyedToken(token)` drops a trailing `#key`, returning `token` unchanged when it has none. `parseToken` requires the closing `>` to be the last character, so classify a possibly-keyed token as `isOpenToken(unkeyedToken(token))` — a key can neither introduce nor remove a hole.
