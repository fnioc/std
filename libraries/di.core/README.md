...
| `LifetimeModel<Lifetime>`                       | The scope/lifetime pattern a container runs on. `@rhombus-std/di`'s `noopLifetimeAddon()` is the built-in model that retains nothing — every registration is constructed fresh every time.                                                                                   |
