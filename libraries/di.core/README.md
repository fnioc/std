...
| `Behavior` / `Hooks`                            | The four hooks one behavior contributes — `beginResolve`, `beforeConstruct`, `canonicalize`, `afterConstruct` — each optional, each written as a plain handler or as middleware with a trailing `next`.                                                                                  |
