...
const IGreeter = Type.named('IGreeter', 'app');
...
| `Type` (re-exported from `primitives`)                      | The address every registration is filed under — `Type.named`, `Type.tag`, `Type.array`, and the rest of the factories.                                                |
