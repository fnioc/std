...
| `Starfish`                                      | The hookable door, resolved like any other service: `bind` hands a provider the function its own resolutions run through, and `register` files an addon's construction hooks.                                                                                                |
| `Audit`                                         | Access to the resolve that constructed the holder — what was asked for, which construction the holder fills, and what encloses it. Name `Audit` in a constructor's signature to receive one.                                                                                 |
| `Diagnostics`                                   | The readings a diagnostics addon has taken for the ask the holder was resolved from. Name `Diagnostics` in a constructor's signature to receive one.                                                                                                                         |
