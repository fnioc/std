...
// abstractions const AND the runtime `HostingHostBuilderAugmentations` both target
...
// (rule 0.6). The runtime `HostingHostBuilderAugmentations` merges its own members
...
// `HostingHostBuilderAugmentations` merge is cross-package, so it must use a
