// ConfigSection -- a pure (root, path) view with zero independent
// storage, plus index-navigable dot/bracket access.
//
// Every read/write routes back through the owning ConfigRoot with the
...
export class ConfigSection extends IndexAccessed<IndexedSection> implements IConfigSection {
  ...
  public constructor(root: ConfigRoot, path: string) {
    super();
    this.#root = root;
    this.#path = path;
  }
 ...
 * ConfigSection and ConfigRoot.
 */
export function subtreeToObject(node: IConfig): ConfigObject {
