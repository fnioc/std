...
| Export                                  | What it is                                                                                                                                             |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ManifestDescriptorAugmentations`          | The type-argument-derived bodies for `add`, `addValue`, `tryAdd`, `tryAddValue`, `replace`, `replaceValue`, `removeAll`, and `describe`. `ManifestDescriptorValueAugmentations` carries `add`/`tryAdd`/`replace`'s value shape as a second set, since one object literal can't declare two same-named members. |
| `AsImplementerDescriptorAugmentations`     | The type-argument-derived bodies for `describe(...).asClass(ctor)` / `.asFactory(fn)` — the implementer's type derived from the value instead of taken explicitly. |
