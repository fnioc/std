// `valueof<T>()` binds a literal type argument and, at build time, becomes that
// type's value — `valueof<"scoped">()` → `"scoped"`, `valueof<42>()` → `42`. It
// backs the `.as<Scope>()` lifetime form, whose body is
// `this.as(valueof<Scope>())`: the type-arg scope name becomes the value-arg
// `as(scope)` takes.
