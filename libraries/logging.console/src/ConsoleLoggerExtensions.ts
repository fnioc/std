 ...
 * (see the module doc). Registered against `tokenfor<ILoggingBuilder>()` below
 * and reachable as the standalone `ConsoleLoggerExtensions.addConsole(builder)`.
 */
export const ConsoleLoggerExtensions = {
  ...
  addConsole(builder: ILoggingBuilder, configure?: Func<[ConsoleLoggerOptions], void>): ILoggingBuilder {
    ...
    if (registration.provider === undefined) {
      registration.provider = new ConsoleLoggerProvider(registration.loggerOptions, [...registration.pendingFormatters,
   ...
   */
  addSimpleConsole(builder: ILoggingBuilder, configure?: Func<[SimpleConsoleFormatterOptions], void>): ILoggingBuilder {
   ...
   */
  addJsonConsole(builder: ILoggingBuilder, configure?: Func<[JsonConsoleFormatterOptions], void>): ILoggingBuilder {
   ...
   */
  addSystemdConsole(builder: ILoggingBuilder, configure?: Func<[ConsoleFormatterOptions], void>): ILoggingBuilder {
...
// Method form: merges onto the owning ILoggingBuilder interface so a consumer
// holding it sees the methods. Concrete implementers (logging's
// LoggingBuilder) inherit these through their `interface ... extends
