...
No decorators. No `reflect-metadata`. No runtime type introspection. Feed it string tokens and dep arrays (generated by `@rhombus-std/di.transformer` or written by hand via `@rhombus-std/di.core`'s authoring surfaces) and it handles the rest.
