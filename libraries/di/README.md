...
`Builder.withDefaults(options?)` is the common-path opener for an application: it seeds the standard lifetime model, always-on address validation, and — each behind an option (`validateScopes`, `validateOnBuild`, both off by default) — scope and buildability validation, then chains like any builder (`Builder.withDefaults().withServices(…).build()`).
...
| what calling the registered callable returns      | — (sugar only)                  | `resolveWith<T, Args>(...args)`      |
| the same, from a promise-returning callable       | — (sugar only)                  | `resolveWithAsync<T, Args>(...args)` |
...
`resolve`, `resolveAsync`, `resolveWith`, `resolveWithAsync`, `instantiate` and `invoke` each have a `try` twin — `tryResolve`, `tryResolveAsync`, `tryInvoke` — answering `undefined` where the plain verb throws, by asking for the same shape beside the `undefined` literal. The async twins settle on `undefined` rather than answering it, so their type is `Promise<T | undefined>`. The aggregate verbs have no twin: nothing registered is an empty collection, never an absence.
...
| Export                                                                                        | What it is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Builder`                                                                                     | The one entry point: `Builder.useAddon(...)` / `Builder.withServices(...)` open a chain, `Builder.withDefaults(...)` opens the common-path chain (standard lifetime model, address validation, and scope/buildability validation each behind an option), `useAddon` / `withServices` extend it, and `build()` seals it into an `IServiceProvider`.                                                                                                                                                                             |
...
| `validateAddresses`                                                                           | The addon reading the addresses themselves — every registration address at build, every ask's address at the door — against `di.core`'s `addressRules`, which name the spellings that are legal but likelier a slip: a doubly-keyed type, a promise of a promise, an aggregate of an optional. Every rule reports a warning, so `warningsAsErrors: true` is what makes one refuse; `suppress` drops rules by id. Every objection it sees, stopping or not, is filed into an `IAddressDiagnostics` the built provider resolves. |
...
| `validateScopes`, `ScopeValidationError`                                                      | The optional layer over the model refusing a scoped registration reached under the singleton scope — resolved from the provider `build()` returns, or consumed by a singleton — with `ScopeValidationError` naming the scoped address.                                                                                                                                                                                                                                                                                         |
| `DiError`, `UnsatisfiableError`, `CycleError`, `UniversalAddressError`, `ObjectDisposedError` | Re-exported from `di.core` — the same classes, so `instanceof` holds whichever package a caller imports the taxonomy from. The validation addons throw the platform's `AggregateError` of these leaves.                                                                                                                                                                                                                                                                                                                        |
