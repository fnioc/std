...
| Export                                                                                                                                                                                                                                                                                                                                                        | What it is                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `ServiceManifest<Scopes>`                                                                                                                                                                                                                                                                                                                                     | The registration builder — `.add()` / `.addFactory()` / `.addValue()` / `.build()`.                              |
| `ServiceProviderClass`                                                                                                                                                                                                                                                                                                                                        | The concrete container implementation backing `ServiceProvider`, exported for white-box use.                     |
| `ServiceProvider<Scopes>` (type)                                                                                                                                                                                                                                                                                                                              | The public container surface: `resolve`, `resolveAsync`, `resolveFactory`, `createScope`, disposal.              |
| `Resolver`, `ScopeFactory`, `RequiredResolver`, `ServiceQuery` (types)                                                                                                                                                                                                                                                                                        | The capability interfaces `ServiceProvider` composes.                                                            |
| `RESOLVER_TOKEN`                                                                                                                                                                                                                                                                                                                                              | The intrinsic token a `Resolver`-typed constructor parameter derives, for factories that want the live provider. |
...
### `add(token, Ctor, signatures?)` — registration-carried dependency signatures
...
| `add(token, ctor, signatures?)`           | `(token: string, ctor, signatures?: readonly (readonly DepSlot[])[]) => AddBuilder`             | Class registration (lowered form). An open (holey) token routes to the open-registration table; `signatures` is the sole signature channel (see [Open generics](#open-generics)).  |
...
- **`@rhombus-std/options`** and **`@rhombus-std/options.augmentations`** — build an `Options<T>` accessor and configuration-binding pipeline on top of a `ServiceManifest`.
...
- The internal scope frame (instance cache + disposal + parent link) is not exported — consumers see only the `ServiceProvider` interface a scope backs, never the frame implementation.
