import { type Addon, type Audit, type Behavior, Control, type IEngineHooks, type LifetimeArgument, Registration, type Request } from '@rhombus-std/di.core';
...
class AuditView implements Audit {
  ...
  get address(): Type | undefined {
    return this.#enclosing().find(Boolean);
  }
...
const hooks: Behavior<AuditCompartment> = {
  ...
  beginResolve(request: Request): AuditCompartment {
    return { request: request.type, frame: undefined };
  },
 ...
 */
export function auditAddon<Lifetime>(...lifetime: LifetimeArgument<Lifetime>): Addon<Lifetime>;
export function auditAddon(lifetime?: any): Addon<any> {
  return {
    registrations: [
      Registration.factory(
        ...
        () => {
          throw new Error(
            `${Type.stringify(typefor<Audit>())} is answered by the audit addon's own hooks, and this container never installed them — resolve it from a container built with useAddon(auditAddon())`,
          );
        },
