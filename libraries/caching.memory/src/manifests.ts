...
export function getMemoryCacheManifest(setup?: Func<[MemoryCacheOptions], void>): Manifest<unknown> {
  ...
  if (setup !== undefined) {
    ...
    m = m.import(getConfigureManifest(MEMORY_CACHE_OPTIONS_TYPE, setup));
  }
  ...
  return m.tryAdd(MEMORY_CACHE_TYPE,
    (resolver: IServiceProvider) => new MemoryCache(resolver.resolve<IOptions<MemoryCacheOptions>>(), resolver.resolve(Type.optional(typefor<ILoggerFactory>())) as ILoggerFactory | undefined),
...
export function getDistributedMemoryCacheManifest(setup?: Func<[MemoryDistributedCacheOptions], void>): Manifest<unknown> {
  ...
  if (setup !== undefined) {
    m = m.import(getConfigureManifest(MEMORY_DISTRIBUTED_CACHE_OPTIONS_TYPE, setup));
  }
  return m.tryAdd(DISTRIBUTED_CACHE_TYPE, (resolver: IServiceProvider) =>
    new MemoryDistributedCache(
      ...
      resolver.resolve(Type.optional(typefor<ILoggerFactory>())) as ILoggerFactory | undefined,
