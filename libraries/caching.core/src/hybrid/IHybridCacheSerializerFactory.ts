...
// value -- the `Type` node derived inline via `typefor<T>()` (or written
// literally by hand).
...
export interface IHybridCacheSerializerFactory {
   ...
   *
   * @param type The `Type` node identifying the type being serialized or
   * deserialized -- the runtime analog of `T`, derived inline via
   * `typefor<T>()`.
