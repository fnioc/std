...
| `MemoryCacheSugarAugmentations`, `CacheEntrySugarAugmentations`, `MemoryCacheEntryOptionsSugarAugmentations`, `DistributedCacheSugarAugmentations`, `DistributedCacheEntryOptionsSugarAugmentations` | The augmentation objects backing the convenience methods above — call them directly (`MemoryCacheSugarAugmentations.get(cache, key)`) if you'd rather not rely on the merged method form.         |

Every convenience method (`cache.get(...)`, `cache.getOrCreate(...)`,
`entry.setPriority(...)`, `options.setAbsoluteExpiration(...)`, and so on) is
also available as a plain function on the matching `*SugarAugmentations` object, in
...
- `IDistributedCache`'s byte-array `set` is standalone-only. Its name is
  already taken by the interface's own `set(key, value, options)` primitive, so
  the default-options form stays a plain function:
  `DistributedCacheSugarAugmentations.set(cache, key, bytes)`. `setString` and
  `getString` merge onto the interface normally and are dot-callable.
