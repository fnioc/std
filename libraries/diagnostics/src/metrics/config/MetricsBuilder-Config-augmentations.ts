...
export namespace MetricsBuilderConfigAugmentations {
  ...
  export function addMetricsConfig<Self extends IMetricsBuilder>(this: Self, config: IConfig): Self {
    this.services = this.services.add(METRICS_CONFIGURE_TYPE, new MetricsConfigureOptions(config), ConstantType);
    this.services = this.services.add(METRICS_CHANGE_TOKEN_SOURCE_TYPE, new ConfigChangeTokenSource(config), ConstantType);
    this.services = this.services.add(METRICS_CONFIGURATION_TYPE, new MetricsConfig(config), ConstantType);
    return this;
  }
}
