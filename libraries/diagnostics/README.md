...
`addMetrics` registers a resolvable `IOptions<MetricsOptions>` assembly and, if
...
| `ServiceManifestMetricsAugmentations`, `ServiceManifestTracingAugmentations`                               | The standalone function form of `addMetrics`/`addTracing`, for callers who don't want the method installed on their builder.                                          |
