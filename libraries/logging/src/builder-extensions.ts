// ILoggingBuilder extension functions, ported from ME.Logging's
// `LoggingBuilderExtensions` / `FilterLoggingBuilderExtensions`.
//
// ILoggingBuilder is an interface THIS family owns (logging.core), so per the
// repo's "explicit form is primary" convention these are plain exported
// functions taking the builder first — no declaration-merging augmentation.
//
 ...
 */
export function addProvider(builder: ILoggingBuilder, provider: ILoggerProvider): ILoggingBuilder {
 ...
 * layer lands.
 */
export function setMinimumLevel(_builder: ILoggingBuilder, _level: LogLevel): ILoggingBuilder {
  throw new Error(
 ...
 */
export function clearProviders(_builder: ILoggingBuilder): ILoggingBuilder {
  throw new Error(
