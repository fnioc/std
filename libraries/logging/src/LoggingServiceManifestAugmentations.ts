...
// interface. Type-parameter lists MUST match each target's declaration
// (TS2428): `IServiceManifestBase` takes `<Scopes, Provider>`,
// `ServiceManifestClass` takes `<Scopes>`.
...
export const LoggingServiceManifestAugmentations = {
  addLogging(manifest: ServiceManifestClass<string>,
    ...
    m = m.addClass(closeToken(ILOGGER_TOKEN_BASE, '$1'), LoggerOfT, [[LOGGER_FACTORY_TOKEN, typeArg(1)]], 'singleton');
    ...
    // `ServiceManifest` — logging services are singleton-only. Narrow the scope
    // phantom here: LoggingBuilder merely stores the manifest and never calls
    // the scope-sensitive `build()`, so the phantom is inert.
    //
