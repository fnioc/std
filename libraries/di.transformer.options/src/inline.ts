...
// type parameter bound from the checker), then the primitive (tokenfor) stage
// lowers the result. The body contains `tokenfor<IOptions<T>>()` and
// `tokenfor<T>()` over an UNBOUND generic, so it must never go through a per-file
...
// (this package peers it), so the base always resolves. `tokenfor` is the
// runtime-leaf token primitive (`@rhombus-std/primitives`); it lowers to inline
// token literals and its import elides.
 ...
 *
 *   addOptions<T>() → this.addOptions(tokenfor<IOptions<T>>(), tokenfor<T>())
 *
 * The two token arguments are RELATIONALLY LOCKED: the first is the closed-generic
 * wrapper `IOptions<T>` token an `IOptions<T>` resolution derives, the second the
 * bare element `T` token any `add<T>()` / `resolve<T>()` derives — both minted from
 * the SAME element derivation, so the registered `IOptions<T>` and the `T` it wraps
 * agree by construction. Lowers byte-identically to the explicit two-token verb.
 *
