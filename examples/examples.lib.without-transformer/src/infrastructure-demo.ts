...
import { DiError, EmptyServiceProvider, RESOLVER_TYPE } from '@rhombus-std/di.core';
...
export function demonstrateNullProvider(): readonly string[] {
  // Held as the `IServiceProvider` INTERFACE, not the concrete class: that is the
  ...
  const nowhere: IServiceProvider = EmptyServiceProvider.instance;
  ...
  const lines: string[] = ['no container at all (EmptyServiceProvider):',
    `  provider reports itself a service: ${nowhere.isService(RESOLVER_TYPE)}`,
