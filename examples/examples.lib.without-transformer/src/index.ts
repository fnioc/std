...
// reading a manifest as data, reasoning about tokens, catching engine errors —
...
// should be a provider. Where a demonstration needed one of those it lives in
// the example apps instead — and the seam it left behind is named at the point
// it moved.
//
// What library code here DOES do with a provider, it does only to something it
...
// The error taxonomy, CLASSIFIED. Every class the engine can throw is a
...
// library cannot do is PROVOKE them, since that takes a built provider, so each
