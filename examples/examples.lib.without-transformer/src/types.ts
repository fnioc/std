...
// `@rhombus-std/examples.contracts` types (`Type.named(exportedName,
// importSpecifier)`). Writing the derived form by hand is what lets this
...
//
// `Type` nodes are INTERNED: this file's `Type.named('IGreeting',
// '@rhombus-std/examples.contracts')` and the with-transformer library's
...
export const GREETING_TYPE: Type = Type.named('IGreeting', '@rhombus-std/examples.contracts');

/** The optional service this library alone registers. */
export const HEALTH_CHECK_TYPE: Type = Type.named('IHealthCheck', '@rhombus-std/examples.contracts');
