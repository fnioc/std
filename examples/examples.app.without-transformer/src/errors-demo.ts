...
function withUnsatisfiableStore(): Manifest<'singleton'> {
  return new DefaultManifest<'singleton'>().addClass(STORE_TYPE, BrokenStore, [[CONNECTION_TYPE]], 'singleton');
}
...
export function demonstrateErrors(): readonly string[] {
  ...
  lines.push(stagedFailure('a dependency cycle', () => {
    ...
    services = services.addClass(LEDGER_TYPE, Ledger, [[AUDIT_TYPE]], 'singleton');
    services = services.addClass(AUDIT_TYPE, AuditLog, [[LEDGER_TYPE]], 'singleton');
...
function ambiguous(): Manifest<'singleton'> {
  ...
  services = services.addClass(REPORT_TYPE, ReportService, [[Type.union(METRICS_TYPE, AUDIT_TYPE)]], 'singleton');
  return services;
}
