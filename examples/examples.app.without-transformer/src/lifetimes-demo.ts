// LIFETIMES, SCOPES AND DISPOSAL — the explicit-token (no-transformer) dialect.
//
...
//
// This is the hand-written half of the pair: every registration names an
// explicit string token and states its dependency signature as plain data, and
// every lookup names the token too. It is exactly what its
// ../../examples.app.with-transformer/src/lifetimes-demo.ts sibling's
// `addClass<IThing>(Thing)` / `resolve<IThing>()` forms lower to. Diff the two
// files: the scenario, the assertions and the printed lines are identical — only
// the authoring dialect differs.
//
// NOTE on the tokens below: unlike the shared `@rhombus-std/examples.contracts`
// types in main.ts, these contracts are PRIVATE to this demonstration and are
// never resolved from another package, so the token strings are simply picked to
// be unique and readable. A token is an opaque string; agreeing on one only
// matters when two packages have to meet at it.
...
const POOL_TOKEN = 'examples.lifetimes:IConnectionPool';
const UNIT_OF_WORK_TOKEN = 'examples.lifetimes:IOrderUnitOfWork';
const OUTBOX_TOKEN = 'examples.lifetimes:IOutboxChannel';
const SEARCH_INDEX_TOKEN = 'examples.lifetimes:ISearchIndexWriter';
const VALIDATOR_TOKEN = 'examples.lifetimes:IOrderValidator';
const REPORT_CACHE_TOKEN = 'examples.lifetimes:IOrderReportCache';
const TRACE_TOKEN = 'examples.lifetimes:ILifecycleTrace';
 ...
 * the same token replaces it without disturbing anything else.
 */
 ...
 * `'singleton'`, `'request'`, and — for the validator — no scope argument at
 * all. There is no `"transient"` tag to write: transient IS the absence of a
 * tag. Argument 3 is the dependency signature, one inner array per constructor
 * overload, listing each parameter's token in order; `[[]]` states "this
 * constructor takes nothing" rather than leaving it to be inferred. The manifest
...
export async function demonstrateLifetimes(): Promise<readonly string[]> {
  const lines: string[] = ['=== di lifetimes and disposal — without transformer ==='];
  ...
  const validatorA = requestOne.resolve<IOrderValidator>(VALIDATOR_TOKEN);
  const validatorB = requestOne.resolve<IOrderValidator>(VALIDATOR_TOKEN);
  ...
  const outboxOne = requestOne.resolve<IOutboxChannel>(OUTBOX_TOKEN);
  const unitOfWorkOne = requestOne.resolve<IOrderUnitOfWork>(UNIT_OF_WORK_TOKEN);
  const poolOne = requestOne.resolve<IConnectionPool>(POOL_TOKEN);
  ...
  const outboxTwo = requestTwo.resolve<IOutboxChannel>(OUTBOX_TOKEN);
  const unitOfWorkTwo = requestTwo.resolve<IOrderUnitOfWork>(UNIT_OF_WORK_TOKEN);
  const poolTwo = requestTwo.resolve<IConnectionPool>(POOL_TOKEN);
  ...
  const requestUnitOfWork = lenientRequest.resolve<IOrderUnitOfWork>(UNIT_OF_WORK_TOKEN);
  const cache = lenientRequest.resolve<IOrderReportCache>(REPORT_CACHE_TOKEN);
  ...
  try {
    strictRequest.resolve<IOrderReportCache>(REPORT_CACHE_TOKEN);
  } catch (error) {
  ...
  try {
    lateRequest.resolve<IOrderUnitOfWork>(UNIT_OF_WORK_TOKEN);
  } catch (error) {
  ...
  flakyRequest.resolve<IOutboxChannel>(OUTBOX_TOKEN);
  flakyRequest.resolve<ISearchIndexWriter>(SEARCH_INDEX_TOKEN);
