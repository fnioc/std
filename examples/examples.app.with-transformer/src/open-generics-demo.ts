 ...
 * bound that the type-driven dialect spells `Hole<1, Entity>`; composing a
...
manifest = manifest.addClass(TABLE_TEMPLATE, InMemoryTable, [[seedOf(HOLE_1), witnessOf(HOLE_1)]], 'singleton');
...
manifest = manifest.addClass(REPOSITORY_TEMPLATE, InMemoryRepository, [[tableOf(HOLE_1), witnessOf(HOLE_1)]],
  'singleton');
...
manifest = manifest.addClass(repositoryOf(AUDIT_EVENT_TYPE), AuditRepository, [[tableOf(AUDIT_EVENT_TYPE)]],
  'singleton');

// Template 3 — fully open, arity 2. Each dependency names a DIFFERENT hole, so
// the two sides close independently.
manifest = manifest.addClass(JOIN_TEMPLATE, RepositoryJoin, [[repositoryOf(HOLE_1), repositoryOf(HOLE_2)]],
  'singleton');
...
// left dependency slot; only the right slot carries a hole. Both templates match
// a join whose left side is an order, and the later registration is the one that
// takes it.
manifest = manifest.addClass(ORDER_JOIN_TEMPLATE, OrderJoin, [[repositoryOf(ORDER_TYPE), repositoryOf(HOLE_2)]],
  'singleton');
