 ...
 *
 * @param resolver The live provider view (the intrinsic `IResolver` token).
 */
export function makeServerReport(resolver: IResolver): IServerReport {
  ...
  const server = resolver.resolve<IOptions<ServerOptions>>().value;
  const policy = resolver.resolve<IOptions<GreetingPolicy>>().value;
