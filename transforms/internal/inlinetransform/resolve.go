...
func Resolve(prog *driver.Program, checker *shimchecker.Checker, ex *bodyExtractor, owned OwnedEntry) (*Resolved, bool, error) {
	...
	kind, ok := e.Kind()
	if !ok {
		return nil, false, fmt.Errorf("INLINE_ENTRY_SHAPE: entry %q is not a certified shape", e.Type)
	}
	...
	if kind == KindFunction {
		return resolveFunction(checker, moduleSym, owned, e, pkg, typeName, body)
	}
...
// resolveFunction resolves a free-function entry: the export symbol's single
// function-like declaration is mapped to the body. An overloaded free function
// (more than one function-like declaration) is specced-not-certified.
func resolveFunction(checker *shimchecker.Checker, moduleSym *shimast.Symbol, owned OwnedEntry, e Entry, pkg, typeName string, body *ResolvedBody) (*Resolved, bool, error) {
	...
	if len(fnDecls) != 1 {
		return nil, false, fmt.Errorf("INLINE_ENTRY_SHAPE: free-function entry %q has %d function-like declarations (overloaded free functions are not certified)", e.Type, len(fnDecls))
	}
