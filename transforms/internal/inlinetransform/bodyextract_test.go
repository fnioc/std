...
func TestExtractPropertyAssignmentAndArrowForms(t *testing.T) {
	inline := `import { nameof } from '@rhombus-std/primitives';
export const QueryInline = {
  bar: function<T>(this: any): boolean { return this.isService(nameof<T>()); },
  baz: <T>(): boolean => { return this.isService(nameof<T>()); },
  qux: <T>(): boolean => this.isService(nameof<T>()),
};
`
