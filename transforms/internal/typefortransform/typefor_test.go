...
// shape: `@rhombus-std/primitives` (the runtime `Type` object) and
...
func buildTypeforWorkspace(t *testing.T, mainSrc string) (*driver.Program, string) {
  ...
  "exports": { ".": { "types": "./src/index.ts", "default": "./src/index.ts" } }
	...
	writeFile(t, filepath.Join(primitives, "src", "index.ts"), `export declare const Type: {
  global(name: string, genericTypes?: unknown[]): unknown;
  import(name: string, from: string, genericTypes?: unknown[]): unknown;
  func(returnType: unknown, ...args: unknown[]): unknown;
  ctor(instanceType: unknown, ...args: unknown[]): unknown;
  tag(type: unknown, tag: string): unknown;
  typeLiteral(value: unknown): unknown;
  generic(label: string): unknown;
...
`
	...
	want := `Type.import("IThing", "@scope/app/main")`
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.import("IThing", "@scope/app/main", [Type.import("IOther", "@scope/app/main")])`
	if got := exprFor(t, out, "tok"); got != want {
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.import("IThing", "@scope/app/main", [Type.generic("1")])`
	if got := exprFor(t, out, "tok"); got != want {
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.tag(Type.import("IThing", "@scope/app/main"), "redis")`
	if got := exprFor(t, out, "tok"); got != want {
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.ctor(Type.import("Foo", "@scope/app/main"))`
	if got := exprFor(t, out, "tok"); got != want {
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.ctor(Type.import("Foo", "@scope/app/main"), Type.import("IA", "@scope/app/main"), Type.import("IB", "@scope/app/main"))`
	if got := exprFor(t, out, "tok"); got != want {
		t.Fatalf("typefor(Foo) = %q, want %q\nfull output:\n%s", got, want, out)
	}
}
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.func(Type.import("IThing", "@scope/app/main"))`
	if got := exprFor(t, out, "tok"); got != want {
		t.Fatalf("typefor<() => IThing>() = %q, want %q\nfull output:\n%s", got, want, out)
	}
}
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `[Type.import("IA", "@scope/app/main"), Type.import("IB", "@scope/app/main")]`
	if got := exprFor(t, out, "tok"); got != want {
...
`
	...
	out := lowerTypefor(t, prog, app)
	want := `Type.import("Foo", "@scope/app/main")`
	if got := exprFor(t, out, "tok"); got != want {
...
`
	...
	if got, want := exprFor(t, out, "inner"), `Type.import("IThing", "@scope/app/main")`; got != want {
...
`
	...
	out := lowerTypefor(t, prog, app)
	wantT := `Type.func(Type.import("IThing", "@scope/app/main"))`
...
`
	...
	out := lowerTypefor(t, prog, app)
	element := `Type.import("IThing", "@scope/app/main")`
