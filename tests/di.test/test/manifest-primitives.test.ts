...
// that name to the same instance: a registration reaches the primitive, and a service type paired
...
describe('add answers to both the primitive and the sugared shapes', () => {
  test('a lone registration reaches the primitive', () => {
    expect(values(blank().add(Registration.value(A, 'a')))).toEqual(['a']);
  });

  test('a service type paired with a constructor reaches the sugar', () => {
    const manifest = blank().add(A, Impl, Type.ctor(A, [[]]), 'singleton');
    expect([...manifest]).toEqual([Registration.ctor(A, Impl, Type.ctor(A, [[]]), 'singleton')]);
  });
...
describe('remove and replace are reachable on the instance', () => {
  test('remove drops the registration it equals', () => {
    const registration = Registration.value(A, 'a');
    const manifest = blank().add(registration).add(Registration.value(B, 'b'));
    expect(values(manifest.remove(registration))).toEqual(['b']);
  });

  test('replace swaps the registration holding the slot, in the slot it holds', () => {
    const manifest = blank()
      .add(Registration.value(A, 'first'))
      .add(Registration.value(B, 'b'));
    expect(values(manifest.replace(Registration.value(A, 'second')))).toEqual(['b', 'second']);
  });
});

describe('a verb that changes nothing returns the receiver', () => {
  test('remove without a match', () => {
    const manifest = blank().add(Registration.value(A, 'a'));
    expect(manifest.remove(Registration.value(B, 'b'))).toBe(manifest);
    expect(manifest.remove(B)).toBe(manifest);
  });

  test('replace without a registration to swap', () => {
    const manifest = blank().add(Registration.value(A, 'a'));
    expect(manifest.replace(Registration.value(B, 'b'))).toBe(manifest);
  });

  test('tryAdd of an already-registered service type', () => {
    const manifest = blank().add(Registration.value(A, 'a'));
    expect(manifest.tryAdd(Registration.value(A, 'again'))).toBe(manifest);
  });
