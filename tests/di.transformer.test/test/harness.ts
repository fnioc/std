...
declare module "@rhombus-std/di.core" {
  ...
  export interface ServiceProvider<S extends string = string> extends Resolver {
    createScope(name?: S): ServiceProvider<S>;
    readonly name: S;
    dispose(): void;
  }
...
const DI_CORE_RECEIVERS = `import type { ServiceManifest, ServiceProvider } from "@rhombus-std/di.core";
declare global {
  const services: ServiceManifest<string>;
  const scope: ServiceProvider<string>;
  const provider: ServiceProvider<string>;
  const root: ServiceProvider<string>;
}
`;
