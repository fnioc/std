...
import { ConfigBuilder, type IndexedSection } from '@rhombus-std/config';
import { XmlStreamConfigSource } from '@rhombus-std/config.xml';
...
function fromXml(text: string): IndexedSection {
  return new ConfigBuilder().add(new XmlStreamConfigSource(text)).build();
}
...
describe('XmlConfigSource file wiring', () => {
