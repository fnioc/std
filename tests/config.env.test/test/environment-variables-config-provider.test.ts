// Behavior tests for EnvironmentVariablesConfigProvider -- loads an
// environment map into the ConfigProvider store, filtering by an
...
import '@rhombus-std/config.env/_/index';
import { EnvironmentVariablesConfigProvider } from '@rhombus-std/config.env/_/EnvironmentVariablesConfigProvider';
import { colonAndDotVariableNameTransformation,
  EnvironmentVariablesConfigSource } from '@rhombus-std/config.env/_/EnvironmentVariablesConfigSource';
...
describe('EnvironmentVariablesConfigProvider with a prefix', () => {
...
describe('EnvironmentVariablesConfigProvider without a prefix', () => {
...
describe('EnvironmentVariablesConfigProvider transform-before-filter order', () => {
...
describe('EnvironmentVariablesConfigProvider purity w.r.t. the injected map', () => {
  test('reload with a different map reflects the new map and drops old keys', () => {
    const source = new EnvironmentVariablesConfigSource({ prefix: 'APP_', env: { APP_A: '1' } });
    const provider = new EnvironmentVariablesConfigProvider(source);
    provider.load();
