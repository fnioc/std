// ConfigProvider base behavior -- exercised through the concrete
// MemoryConfigProvider. The case-insensitive, casing-preserving store
...
import { MemoryConfigProvider, MemoryConfigSource } from '@rhombus-std/config';
import { describe, expect, test } from 'bun:test';

function providerOf(data: Record<string, string>): MemoryConfigProvider {
  return new MemoryConfigProvider(new MemoryConfigSource({ initialData: data }));
}

describe('ConfigProvider store (case-insensitive, casing-preserving)', () => {
...
describe('ConfigProvider.getChildKeys', () => {
