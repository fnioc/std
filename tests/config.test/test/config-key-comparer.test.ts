// compareConfigKeys -- the segment-by-segment, numeric-aware ordering
// that gives config keys (especially array indices) a natural sort. This
// behavior is NEW relative to the pre-rewrite single-Map ConfigRoot,
// which had no comparer at all and enumerated keys in insertion order.

import { compareConfigKeys } from '@rhombus-std/config';
import { describe, expect, test } from 'bun:test';

const cmp = compareConfigKeys;
const sign = (n: number): number => (n < 0 ? -1 : n > 0 ? 1 : 0);

describe('compareConfigKeys', () => {
...
describe('compareConfigKeys delimiter-run collapsing (MEC parity)', () => {
