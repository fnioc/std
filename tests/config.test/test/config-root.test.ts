// ConfigRoot -- the provider-list engine. Every assertion here targets
// behavior the pre-rewrite single-Map ConfigRoot could NOT exhibit:
 ...
 * ConfigProvider.onReload swap-on-fire discipline so
 * ChangeToken.onChange re-registration behaves as in production.
 */
class SwappableProvider implements IConfigProvider {
  #token = new ConfigReloadToken();
  public disposed = 0;
  ...
  public triggerReload(): void {
    ...
    this.#token = new ConfigReloadToken();
    previous.onReload();
  }
...
function rootOfLayers(...layers: Record<string, string>[]): IConfigRoot {
  ...
  return builder.build() as unknown as IConfigRoot;
}

describe('ConfigRoot.get (reverse-order precedence)', () => {
...
describe('ConfigRoot.set (writes to every provider)', () => {
  ...
  test('throws when there are no providers -- nowhere to store the value', () => {
    const root = new ConfigBuilder().build();
    expect(() => root.set('Any:Key', 'value')).toThrow();
  });
});

describe('ConfigRoot.getSection (always returns, never null)', () => {
...
describe('ConfigRoot[Symbol.dispose] (releases registrations + disposable providers)', () => {
