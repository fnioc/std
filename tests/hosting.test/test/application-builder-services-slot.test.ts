...
test('builder.logging registrations reach the manifest build() reads', () => {
  ...
  const providers: ILoggerProvider[] = di.usingLifetimeModel(noopLifetimeAddon()).usingManifest(builder.services).build().resolve(Type.array(LOGGER_PROVIDER_TYPE));
...
test('builder.services and builder.logging registrations both survive into the host', () => {
  ...
  expect(host.services.resolve(Type.from('test:First'))).toBe('first');
  expect(host.services.resolve(Type.from('test:Second'))).toBe('second');
  expect(host.services.resolve(Type.array(LOGGER_PROVIDER_TYPE))).toContain(marker);
...
test('builder.metrics shares the same slot as builder.services', () => {
  ...
  expect(di.usingLifetimeModel(noopLifetimeAddon()).usingManifest(builder.services).build().resolve(Type.from('test:ViaMetrics'))).toBe('yes');
});
...
test('asHostBuilder() replays its delegates into the live slot, not a snapshot', () => {
  ...
  const host = builder.build();
  expect(host.services.resolve(Type.from('test:Early'))).toBe('early');
  expect(host.services.resolve(Type.from('test:Late'))).toBe('late');
