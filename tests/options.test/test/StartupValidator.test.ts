...
// a hand-built fake IServiceProvider so the unit stays independent of the DI
...
describe('StartupValidator.validate', () => {
  test('all targets pass -> does not throw, forcing each value', () => {
    ...
    const validator = new StartupValidator(resolverOf({ a: passing, b: passing }), [Type.from('a'), Type.from('b')]);
  ...
  test('a single failure rethrows that OptionsValidationError as-is', () => {
    ...
    const validator = new StartupValidator(resolverOf({ a: failing(failure) }), [Type.from('a')]);
  ...
  test('a non-validation error propagates immediately, unwrapped', () => {
    ...
    const validator = new StartupValidator(resolverOf({ a: failing(boom) }), [Type.from('a')]);
